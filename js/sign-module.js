...
if(ayralLocation.match(/register/)){
    today = yyyy+'-'+mm+'-'+dd;
...
if(username!=null){
  Prepare('$a-username, $username').setValue(username);
}

function resendActivationPIN(e){
    var inUsername=Prepare('$a-username'),
...
function resendResetKey(e){
  var username=storage.getItem('username');
    if(username!='null' && username!='' && username!=null){
        ...
        var httpReq=new ayralHttpRequest('POST',hostname+'/server/app.sign-module.php?request=ACTIVATION_CODE&id='+username,'default',true);
                  ...
                  try{
                   var result=response.target.responseText;
                     ...
                     if(result.match(/success/)){
                           Prepare(e).setText('Resend Now');
                           alert('Sent Successfully');
                     }else{
                        Prepare(e).setText('Resend');
                        errorAlert('Unable to resend your Activation code!'); 
                     }
                 }
                 catch(error){
                    Prepare(e).setText('Resend');
        ...
        Prepare(e).setText('Resend Now');
        warningAlert('Unable to resend your Activation code !');
    }
}
...
(function(){
    ...
    app.find('$username,$p-new_username,$a-username,$s-username,$r-username').on('keyup',function(){
        var value=Prepare(this).getValue();
    ...
    app.find('$password,$s-password,$p-password,$p-confirm,$b-confirm').on('keyup',function(){
        var value=Prepare(this).getValue();
    ...
    app.find('$signup-trigger').on('click',function(){
        ...
        if(name!=='' && count(name)>3 && email!=='' && count(email)>3 && number!=='' && count(number)>3 && password!=='' && count(password)>6 && confirm!=='' && count(confirm)>6 && username!=='' && count(username)>=8 && Date.parse(dob) && dob!==null && dob!=='' && dob!==0 && address!=='' && count(address)>2){
            ...
            if(isMail(email) && isNumbers(number) && password===confirm){
                ...
                //test terms
                    ...
                    blockProgress();
                      ...
                      var httpReq=new ayralHttpRequest('POST',hostname+'/server/app.sign-module.php?objective=1&request=SIGNUP','default',true);
                      httpReq.execute(function(response){
                        if(response=='progress'){
                         controller.setHtml('<img src="images/loader.gif" class="btn-loader">');
                        }else{
    ...
    app.find('$activation-trigger').on('click',function(){
        var inUsername=Prepare('$a-username'),
            inCode=Prepare('$a-code'),
            username=inUsername.getValue(),
         ...
         if(code=='' || count(code)<6 ||  notNumbers(code)){
            inCode.setError();
         }else{
            inCode.removeError(); 
         }
        if(username!=='' && count(username)>=8 && code!=='' && count(code)>=6 && isNumbers(code)){

           var httpReq=new ayralHttpRequest('POST',hostname+'/server/app.sign-module.php?request=ACTIVATION','default',true);
                httpReq.execute(function(response){
                    if(response=='progress'){
                     controller.setHtml('<img src="images/loader.gif" class="btn-loader">');
                    }else{
