...
$request=$app->upper(($_GET['request']));
 ...
 if($request=='CHOOSER'){
  ...
  if(isset($_FILES['fileChooser']['tmp_name'])){
      ...
      $location=$app->capitalize($_POST['i-location']);
      $area=$app->capitalize($_POST['i-area']); 
      ...
      $gender=$app->upper($app->real($_POST['i-gender'])); 
      $discount=$app->upper($app->real($_POST['i-discount'])); 
      $discount=$app->upper($app->real($_POST['i-discount'])); 
              ...
              if($res==1){  
                     ...
                     if($app->count($userInfo)>0){
                        if($row=$app->fetch($userInfo)){
                            $user_name=$app->capitalize($app->abs($row['NAME']));  
                            $user_mobile=$app->lower($row['MOBILE_NUMBER']);
