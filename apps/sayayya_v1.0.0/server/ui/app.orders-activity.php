...
$uid=$app->real($_GET['id']);
    ...
    if($type=='N' and $resultSet==1){
       $OrderDetails=$app->find("{owner_id='$uid' and (status ='OA' or status ='PM' or status ='OR') order by LAST_MODIFIED_DATE asc limit 25}",'s_orders'); 
    }
    if($type=='D' and $resultSet==2){
       $OrderDetails=$app->find("{id='$uid' and (status ='PM' or status ='OA' or status ='OR' or status ='OC') order by LAST_MODIFIED_DATE asc limit 25}",'s_orders'); 
    }
    if($type=='H' and $resultSet==3){
       $OrderDetails=$app->find("{(id='$uid' or owner_id='$uid') and (status ='PM' or status ='OA' or status ='OR' or status ='OC' or status ='AC')  order by LAST_MODIFIED_DATE asc limit 25}",'s_orders');    
    }
